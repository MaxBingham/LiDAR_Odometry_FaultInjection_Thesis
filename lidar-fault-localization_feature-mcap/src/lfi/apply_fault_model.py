...
from lfi.fault_models.rain_injector import RainSimulator
...
def apply_fault_model(
    ...
    Args:
        points: (N, 3) or (N, 4) array [x, y, z] or [x, y, z, intensity]
                If 3 columns provided, intensity=0.5 is auto-added for processing
    ...
    Returns:
        Array with same shape as input (3 or 4 columns)
    """
    ...
    # Handle 3-column input (auto-add intensity)
    if points.shape[1] == 3:
        intensity = np.full((points.shape[0], 1), 0.5, dtype=points.dtype)
        points = np.hstack([points, intensity])
        had_intensity = False
    elif points.shape[1] == 4:
        had_intensity = True
    else:
        raise ValueError(f"points must have 3 or 4 columns, got {points.shape[1]}")
    ...
    if faultmodel == "fog":
        ...
        # Keep last frame's intensity stats
        if sim.stats.get("intensity_atten_mean", 0.0) > 0:
            _FAULT_STATS["intensity_atten_mean"] = sim.stats.get("intensity_atten_mean", 0.0)
            _FAULT_STATS["intensity_atten_median"] = sim.stats.get("intensity_atten_median", 0.0)
            _FAULT_STATS["intensity_atten_std"] = sim.stats.get("intensity_atten_std", 0.0)
        save_fault_stats()

        # Return same shape as input
        return result if had_intensity else result[:, :3]
...
def save_distance_stats(visibility: float, fog_metric: str = "distance") -> None:
    ...
    sim = get_fog_simulator(visibility, fog_metric)
    if sim is not None:
