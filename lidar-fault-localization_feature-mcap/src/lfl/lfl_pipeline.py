...
def process_fog_only(args):
    """Process fog model without KISS-ICP or EVO - metrics only."""
    import sys
    from pathlib import Path
    from tqdm import tqdm
    from lfi.apply_fault_model import save_fault_stats, save_distance_stats
    
    # Add kiss_icp_modifications to path
    kiss_icp_path = Path(__file__).parent.parent.parent / "kiss_icp_modifications"
    if str(kiss_icp_path) not in sys.path:
        sys.path.insert(0, str(kiss_icp_path))
    
    from kiss_icp.datasets.kitti import KITTIOdometryDataset
    
    # Create dataset (fault model applied in read_point_cloud)
    ...
    # Process all scans (triggers fault model application)
    print(f"Processing {len(dataset)} scans with fog model (V={args.visibility}m)...")
    for idx in tqdm(range(len(dataset)), desc="Applying fog"):
        _ = dataset[idx]  # Triggers read_point_cloud -> apply_fault_model
    
    # Save accumulated stats to file
    save_fault_stats()
    
    # Save distance statistics for fog
    if args.visibility is not None:
        fog_metric = getattr(args, 'fog_metric', 'distance')
        save_distance_stats(args.visibility, fog_metric)
    
    return None  # No result_dir for fog-only mode


def run_lfl(args):
    """Run one experiment and return stats dict."""

    from lfi.apply_fault_model import reset_fault_stats, save_distance_stats
    reset_fault_stats()

    # Fog-only mode: skip KISS-ICP and EVO entirely
    if args.fault_model == "fog":
        result_dir = process_fog_only(args)
        
        # Collect stats from fault model
        from lfi.apply_fault_model import load_fault_stats, load_distance_stats
        stats = load_fault_stats()
        distance_stats = load_distance_stats()
    ...
    # Save distance statistics for fog runs
    if args.fault_model == "fog" and args.visibility is not None:
        fog_metric = getattr(args, 'fog_metric', 'distance')
        save_distance_stats(args.visibility, fog_metric)
