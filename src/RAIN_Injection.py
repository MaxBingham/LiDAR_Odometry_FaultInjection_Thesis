...
class RainSimulator:
    ...
    def apply_noise(self, points: np.ndarray) -> np.ndarray:
        # Step 1: Validate input - ensure we have a valid (N,3) point cloud array
        ...
        if n_surf > 0:
            ...
            sigma_turb = self.a_turb * np.exp(-self.b_turb * dist[surf_mask]) + self.c_turb
            dr = self.rng.normal(0.0, sigma_turb, size=n_surf)
